...
import { DealDetails } from '../../network/models/deals';
import Chronos from '../../utilities/chronos';
import { useUserStore } from '../../stores/stores';
import useRefreshScreens from '../../hooks/refreshscreens';
import { ContactTypes, NormaliseContact } from '../bottomnavpages/network';
...
function RFQCard({ rfq, role, status, type, isInactiveDeal, dealid, dealcreateddate = null, history = [] }) {
    ...
    return (
        <View>
            ...
            {/* { type === DealTypes.Selling && !isInactiveDeal && <MenuButton label='Edit Tags'/> } */}
            {/* { type === DealTypes.Selling && !isInactiveDeal && <MenuButton label='Edit Deal'/> } */}
...
function DealDetailsPage(props) {
    ...
    const fetchDealDetails = async () => {
        try {
            ...
            const dealDetails = await DealDetails({ dealid: dealid, role: role, rfqid: rfqid });
            populateDataFromDealDetails(dealDetails, rfqid, setCurrentDeal);
        }
        catch (error) {
    ...
    return (
        <ScrollView contentContainerStyle={[styles.page, { paddingBottom: insets.bottom }]} showsVerticalScrollIndicator={false}>
            ...
            />
            {
                currentDeal.dealType === DealTypes.Selling && currentDeal.buyerInformation && !currentDeal.isInactiveDeal &&
                <React.Fragment>
                    <MenuButton label='View Buyer Information' onPress={() => navigation.navigate('ContactDetails', { contact: currentDeal.buyerInformation, title: 'View Buyer Information' })} />
                    <View style={styles.vspace} />
                </React.Fragment>
            }
            {
            ...
            <MenuButton disabled={currentDeal.quotationList.length === 0 || false} label='View Purchase Orders' onPress={() => navigation.navigate('ViewPurchaseOrders')} />
            <MenuButton disabled={currentDeal.purchaseOrdersList.length === 0 || false} label='View Deal Invoices' onPress={() => navigation.navigate('ViewInvoices')} />
            {/* <MenuButton disabled={currentDeal.invoicesList.length === 0 || currentDeal.isInactiveDeal || false} label='View Payments' onPress={() => navigation.navigate('ViewPayments')} /> */}
            {
            ...
            <View style={styles.vspace} />
            {/* <MenuButton disabled={currentDeal.isInactiveDeal} label='Chat with Deal Manager' onPress={() => createAlert(Alert.Info('This feature is coming soon, please stay tuned for updates to the app.', 'Coming Soon'))} onPress={() => navigation.navigate('Chat', { deal: currentDeal.id, dealmanager: currentDeal.dealmanager })} /> */}
