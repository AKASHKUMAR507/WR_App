...
function SearchList({
    ...
    return (
        <View style={inputStyles.inputContainer} onTouchStart={onTouchStart}>
...
function SearchListSheetChildTextComponent({ option, dataKey, selected }) {
    return (
        <View style={[inputStyles.inputWrapper, styles.option, selected && styles.selectedOption]}>
            <Text style={styles.dropdownInput}>{option[dataKey]}</Text>
        </View>
    )
}
...
function SearchListSheetChildWrapper({ item, dataKey, listComponent, handleChange, value, valueComparator }) {
    return (
        <TouchableOpacity activeOpacity={0.8} onPress={() => handleChange(item)}>
            {React.cloneElement(listComponent || <SearchListSheetChildTextComponent />, {
                option: item,
                dataKey,
                selected: valueComparator ? valueComparator(item, value) : isItemSelected(item, value),
            })}
        </TouchableOpacity>
    )
}
