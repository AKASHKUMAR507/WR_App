...
function QuotationsCard({ quotation, dealType }) {
    ...
    return (
        <View>
            ...
            {quotation.lineitems.length > 0 && <MenuButton label='View Line Items' onPress={() => navigation.navigate('ViewLineItems', { lineitems: quotation.lineitems })}/> }
            {
        ...
        <React.Fragment>
            {
                ...
                <ScrollView contentContainerStyle={[styles.page, { paddingBottom: insets.bottom }]} showsVerticalScrollIndicator={false}>
                    { currentDeal.quotationList.map((quotation, index) => <QuotationsCard key={index} quotation={quotation} dealType={currentDeal.dealType}/>) }
                </ScrollView> :
