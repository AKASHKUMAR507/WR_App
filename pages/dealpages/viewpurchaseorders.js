...
function NormalisePurchaseOrders(purchaseOrder, type) {
    const normalisedPO = {
        ...
        poid: purchaseOrder.buyerPoId || purchaseOrder.worldRefPOId,
    }

    normalisedPO.attachments = type === DealTypes.Buyer ? purchaseOrder.poattachmentList : purchaseOrder.attachMentLists;
    normalisedPO.lineItems = type === DealTypes.Buying ? purchaseOrder.lineItems : [];
    normalisedPO.paymentTerms = type === DealTypes.Buying ? purchaseOrder.paymentTerms : [];
    normalisedPO.additions = type === DealTypes.Buying && purchaseOrder.additions;
    normalisedPO.services = type === DealTypes.Buying && purchaseOrder.services;

    return normalisedPO;
}

function NormalisePurchaseOrderLineItem(purchaseOrderLineItems) {
    const normalisedPOListItems = purchaseOrderLineItems.map(lineItems => ({
        deliveryby: lineItems.deliveryDate,
        lineitem_type: lineItems.lineItemType,
        hsn_code: lineItems.hsnCode,
        item_name: lineItems.item_name,
        item_desc: lineItems.description,
        quantity: lineItems.quantity,
        uom: lineItems.quantityType,
        coo: lineItems.coo,
        outputunitrate: lineItems.unitRate,
        outputtotal: lineItems.lineItemTotal
    }));

    return normalisedPOListItems;
}

const normalisedPOAttachment = (data) => {
...
function PurchaseOrdersCard({ purchaseOrder }) {
    ...
    return (
        <View>
            {currentDeal.dealType === DealTypes.Buying ? (
                <View>
                    <View style={styles.card}>
                        <View style={styles.cardHeader}>
                            ...
                            <View style={styles.row}>
                                ...
                                <View style={{ width: 8 }} />
                                <Text style={styles.cardTextBold}>{purchaseOrder.poid}</Text>
                            </View>
                        </View>
                        <View style={styles.cardBody}>
                            <Text style={styles.cardTitle}>{purchaseOrder.sellerDetails.sellername}</Text>
                    ...
                    {purchaseOrder.lineItems.length > 0 && <MenuButton label='View Line Items' onPress={() => navigation.navigate('ViewLineItems', { lineitems: NormalisePurchaseOrderLineItem(purchaseOrder.lineItems) })} />}
                    {purchaseOrder.additions.length > 0 && <MenuButton label='Additional Charges' onPress={() => navigation.navigate('ViewAdditional', { additional: purchaseOrder.additions })} />}
                </View>
            ) : (
                <View>
                    <View style={styles.card}>
                    ...
                    {purchaseOrder?.attachments?.length > 0 && <MenuButton label='View Purchase Order Attachments' onPress={() => navigation.navigate('ViewAttachments', { attachments: normalisedPOAttachment(purchaseOrder.attachments), pageTitle: 'View PO Attachments' })} />}
                </View>
            )}
            {/* {purchaseOrder?.attachments?.length > 0 && <MenuButton label='View Purchase Order Attachments' onPress={() => navigation.navigate('ViewAttachments', { attachments: normalisedPOAttachment(purchaseOrder.attachments), pageTitle: 'View PO Attachments' })} />} */}
            {/* {currentDeal.dealType === DealTypes.Buying && <MenuButton label='View Line Items' onPress={() => navigation.navigate('ViewLineItems', { lineitems: purchaseOrderLineItems.flat(1) })} />} */}
...
function ViewPurchaseOrdersPage(props) {
    ...
    return (
        <React.Fragment>
            ...
            {
                currentDeal?.dealType === DealTypes.Selling && currentDeal.status === "QUOTED" &&
                <React.Fragment>
                    <View style={{ paddingBottom: insets.bottom + 64 }} />
                    {
                        currentDeal.purchaseOrdersList.length === 0 &&
                        <View style={[styles.buttonContainer, { paddingBottom: insets.bottom + 12 }]}>
                            <Button size={ButtonSizes.medium} label='Add New Purchase Order' onPress={() => navigation.navigate('AddPurchaseOrders')} />
                        </View>
                    }
                </React.Fragment>
            }
